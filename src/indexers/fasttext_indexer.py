...
class FastTextIndexer(BaseIndexer):
    """Indexer for fasttext model"""
    ...
    def __init__(
        ...
        """
        :param model: a fasttext-like model
    ...
    def _get_token_vector(self, token: str) -> Optional[np.ndarray]:
        ...
        try:
            # fasttext can sometimes produce vectors via subwords.
            return np.asarray(wv.get_vector(token), dtype=np.float32)
    ...
    def build_index(self, texts: list[str]) -> None:
        ...
        if not texts:
            # create doc zeros array with shape of fasttext model vector size
        ...
        for doc_id in range(len(texts)):
            ...
            # for each token render fasttext embeddings and add to weight sum
    ...
    @classmethod
    def load(cls, in_dir: Path, model: Any, **kwargs) -> "FastTextIndexer":
        """
        Load index artifacts for fasttext
        :param in_dir: directory with index artifacts
        :param model: fasttext-like embedding model (used for query vectors)
        ...
        tfidf_max_features = meta["tfidf_max_features"] if "tfidf_max_features" in meta else 50000
        min_score = meta["min_score"] if "min_score" in meta else 0.0
        ...
        if instance.tfidf_vectorizer is None:
            raise ValueError("TF-IDF vectorizer is empty after loading")
